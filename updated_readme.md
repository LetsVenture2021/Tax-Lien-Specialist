...
## 🚀 Overview
The **Tax Lien Strategist** is a full‑stack, agentic, AI‑powered investment analysis platform designed to
acquire, analyze, and manage tax lien assets with precision. It combines:
...
This README is a high‑level operational guide to the system architecture, development environment, and
core workflows.
...
- Document generation through **Document Agent** with redacted LLM prompts/responses
...
---
## 🔌 API Specification
Full HTTP API contracts are available in:
```
docs/backend_api_specification.md
```
Includes endpoint definitions for:
- Auth
- Investors
- Properties
- Liens
...
- Deal scoring & LTV/yield computation
...
Structured JSON schemas for agent input/output and tool calls live in:
```
...
Covers ingestion, valuation refresh, NAV updates, document generation, reasoning archival, and more.
...
---
## 🚀 Deployment
Deployment instructions for Azure and GCP are documented separately:
```
...
CI/CD pipelines are defined in:
```
...
Located in:
```
docs/seed_data_and_test_fixtures.md
```
Used for spinning up a fresh dev environment.

---
## 🔍 Observability
Guidelines are in:
```
docs/logging_monitoring_and_slos.md
```
Includes:
...
Found in:
```
docs/incident_response_runbooks.md
```
